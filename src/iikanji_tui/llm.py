"""E2 PR-D-c: client-tui のクライアント完結 E2EE フロー用 LLM ヘルパー。

iikanji (client-py) の同名モジュールを移植。サーバ側
ai_receipt.analyze_and_suggest (E2 PR-C-4i で削除済) と等価の Round 1 +
Round 2 ロジック + OpenAI / Anthropic / Google 呼出を Python で再実装。

設計:
- TUI オーナーがローカルに LLM API キーを保持する前提
  (サーバ E2EE blob の復号は browser SharedWorker でのみ可能)
- prompt 材料は /api/v1/ai/prompt-context から取得 (サーバ実装と整合)

将来的に iikanji を依存追加すればこのファイルを削除して iikanji.llm を
使う形に統一できる (E2-D-e 候補)。
"""
